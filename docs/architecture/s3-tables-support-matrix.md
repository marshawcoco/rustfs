...
| Strong snapshot rolling compatibility | Supported | Reads versions 1, 2, and 3; writes version 1 by default. Version 2 and version 3 each require their matching request and fleet-confirmation environment flags. A process preserves the highest observed format and rejects rollback; old readers reject version 3 and its archive index. See the [cutover runbook](../operations/s3-tables-cutover-runbook.md). |
| Durable catalog capacity | Supported / bounded | Warns at 48 MiB and rejects ordinary snapshot growth above 56 MiB, reserving headroom below the 64 MiB hard limit for governance. Global-authorized capacity and bounded receipt-compaction endpoints are available on both REST prefixes. Reads, replay, and non-growing writes remain available at admission pressure. |
| Durable receipt archival | Fleet-gated | Version 3 retains recent commits online and indexes older confirmed receipts in immutable, checksum-verified objects before publishing the root by CAS. Staged recovery history is not trimmed. Active table identities retain ID lookup; archive objects are not automatically garbage-collected and must be included in backups. This bounds the online snapshot, not total physical history storage. |
